...
function getCartTotalAmount() {
  ...
  for (const shoppingCard of SHOPPING_CART) {
    for (const product of PRODUCTS) {
      if (shoppingCard.id === product.id) {
        result += shoppingCard.quantity * product.price;
      }
    }
  }
  return result;
...
function removeProductFromCart(productId) {
  ...
  for (let key in SHOPPING_CART) {
    if(SHOPPING_CART[key].id===productId && SHOPPING_CART[key].quantity>1){
      SHOPPING_CART[key].quantity--;
    }
    if(SHOPPING_CART[key].id===productId && SHOPPING_CART[key].quantity===1){
      SHOPPING_CART.splice(product.id, 1);
    }
  }
}
function removeProductFromCart(productId) {
